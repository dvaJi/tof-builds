...
async function main() {
  const res = await fetch("https://toweroffantasy.info/simulacra");
  ...
  for await (const url of links) {
    ...
    fs.writeFileSync(
      path.join(DATA_PATH, "simulacra", id + ".json"),
      JSON.stringify(charData.props.pageProps.simulacrum, undefined, 2)
    );
  }
}
