...
async function main() {
  const res = await fetch("https://toweroffantasy.info/matrices");
  const data = await res.text();
  const dom = new JSDOM(data);
  const charsEl = dom.window.document.querySelectorAll(
    "main > section:nth-child(3) > menu > li > a,main > section:nth-child(4) > menu > li > a,main > section:nth-child(5) > menu > li > a,main > section:nth-child(6) > menu > li > a"
  );
  ...
  for await (const url of links) {
    ...
    fs.writeFileSync(
      path.join(DATA_PATH, "matrices", id + ".json"),
      JSON.stringify(charData.props.pageProps.matrix, undefined, 2)
    );
  }
}
